...
void Arm_Move::arm_register(std::string arm_group)
{
    ...
    arm_group_->setEndEffectorLink("robotiq_85_base_link");
...
void Arm_Move::hand_close()
{
    ...
    // ROS_INFO_STREAM("*****************");
    show_value<double>(joint_value);
    double move_range = close_range_ - joint_value[1];
    double sampling_range = move_range / 10;
    ros::Rate loop(20);
    for (int i = 0; i < 15; i++) {
        for (int j=0; j < joint_value.size(); j++)
        {
        joint_value[j] += sampling_range;
        }
...
geometry_msgs::TransformStamped Arm_Move::get_pose_tf(std::string source, std::string target, double z_temae)
{
    ...
    q_moto = Arm_Move::convert_quat(q_y, q_moto, -M_PI / 1.98)*q_moto;
    // q_moto = Arm_Move::convert_quat(q_x, q_moto, -M_PI);
    rotate_at = q_moto;
